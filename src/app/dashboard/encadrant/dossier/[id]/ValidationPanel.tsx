...
export default function ValidationPanel({
  ...
  function decide(decision: "APPROUVE" | "REFUSE" | "CORRECTION_DEMANDEE") {
    startTransition(async () => {
      setError(null)
      const result = await soumettreDecision(dossierId, decision, commentaire)
      if (result.error) {
      ...
      <div className="px-5 py-5">
        <div className="mb-4">
          ...
          <textarea
            ...
            className={
              "w-full resize-y rounded border border-border bg-white px-3 py-2 text-sm " +
              "text-foreground placeholder:text-muted/60 outline-none transition-colors " +
              "min-h-[90px] focus-visible:ring-2 focus-visible:ring-accent focus-visible:ring-offset-1"
            }
          />
        </div>
