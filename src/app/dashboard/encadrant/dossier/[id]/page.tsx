...
export default async function EncadrantDossierPage({
  params,
}: {
  ...
  if (!encadrant) redirect("/dashboard/encadrant")

  const dossier = await prisma.dossier.findFirst({
    where: { id, encadrantId: encadrant.id },
    include: {
      doctorant: {
        include: {
  ...
  return (
    <div className="max-w-2xl">
          ...
          dossier.publications.map((p) => (
            <div key={p.id} className="border-b border-border py-3 last:border-b-0">
              ...
              <p className="text-xs text-muted">
                {PUB_TYPE_LABEL[p.type] ?? p.type}&nbsp;·&nbsp;{p.auteurs}&nbsp;·&nbsp;{p.annee}
                {p.revue && <>&nbsp;·&nbsp;{p.revue}</>}
      ...
      {/* Action panel — only when awaiting this encadrant's decision */}
